...
import { FileUp, Cpu, CheckCircle2, Sparkles, ShieldAlert } from 'lucide-react';
...
export const DocumentParser: React.FC<Props> = ({
  ...
  const handleFileUpload = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      ...
      if (content) {
        const { sanitizedText, isThreatDetected, warningMessage } = sanitizeInput(content, 200000);
        if (isThreatDetected && warningMessage) {
          setSecurityWarning(warningMessage);
  ...
  return (
    <section className="bg-slate-900 border border-slate-800 rounded-2xl p-5 mb-6 shadow-xl">
      {/* Security Threat Warning Banner */}
      {securityWarning && (
        <div className="mb-3 flex items-center gap-2 bg-red-950/80 border border-red-800 text-red-300 text-xs font-semibold rounded-xl px-4 py-2.5 animate-fade-slide-up">
          <ShieldAlert className="w-4 h-4 text-red-400 shrink-0" />
          <span>🛡️ Security: {securityWarning}</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 flex flex-col sm:flex-row gap-2">
          <label htmlFor="nexus-custom-contract-input" className="sr-only">
            Paste custom contract clauses
          </label>
          <textarea
            id="nexus-custom-contract-input"
            aria-label="Paste custom contract clauses"
