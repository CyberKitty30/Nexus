...
export const StatutoryGuardrailModal: React.FC<Props> = ({
  ...
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="guardrail-modal-title" className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-amber-500/60 w-full max-w-2xl rounded-2xl p-6 shadow-2xl space-y-5">
        <div className="flex items-center justify-between pb-3 border-b border-slate-800">
          <div className="flex items-center gap-2 text-amber-400">
            ...
            <h3 id="guardrail-modal-title" className="text-base font-bold text-white">Statutory Uncertainty Guardrail</h3>
          </div>
          <button onClick={onClose} aria-label="Close guardrail window" className="p-1 hover:bg-slate-800 rounded-lg text-slate-400 hover:text-white cursor-pointer">
            <X className="w-5 h-5" />
          </button>
        </div>
