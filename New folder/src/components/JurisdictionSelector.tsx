...
export const JurisdictionSelector: React.FC<Props> = ({
  ...
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="jurisdiction-modal-title" className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-700 w-full max-w-4xl rounded-2xl p-6 shadow-2xl max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center pb-4 border-b border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <h2 id="jurisdiction-modal-title" className="text-lg font-bold text-white">Global Jurisdiction & Statutory Engine</h2>
          </div>
          <button onClick={onClose} aria-label="Close jurisdiction selector" className="p-1 hover:bg-slate-800 rounded-lg text-slate-400 hover:text-white cursor-pointer">
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          {Object.values(SUPPORTED_COUNTRIES).map((country) => {
            ...
            return (
              <div
                ...
                role="button"
                tabIndex={0}
                aria-label={`Select ${country.name} legal framework`}
